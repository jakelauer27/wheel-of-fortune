const updateDom = {
  ...
  countWordLength(puzzle) { /////////Returns an Array with Objects that have the letter count for each word in the array
    var words = [{word1: 0}];
    var wordCount = 1;
  ...
  ////////This function displays the puzzle on the game board, centering each word of the puzzle on a new line
  displayNewPuzzle(puzzle) {   

    ///// Array with an object for each word containing the letter count for that word
    var solutionDetails = this.countWordLength(puzzle.solution); 
    const tiles = $('.tile');
    var words = 1;

    /////Sets the starting tile for the first word: Equals the (row length - word length) / 2. This centers the word on the row.
    var nextRow = Math.floor((14 - (solutionDetails[0].word1)) / 2);  

    ////This conditional sets the starting row to the second row if the solution has 1 or 2 words.
    if (solutionDetails.length <= 2) {
      nextRow += 14;
    }

    /////For loop updating each tile on the dom.
    for (let i = 0; i < puzzle.solution.length; i++) {
      let tileIndex = nextRow + i;

      ////Only reveals tiles that aren't spaces
      if (puzzle.solution[i] !== ' ') {
        ...
        //////Displays &, -, and ' characters on the puzzle
        if (puzzle.solution[i] === '&' || puzzle.solution[i] === '-' || puzzle.solution[i] === "'") {
          $(tiles[tileIndex]).removeClass('hide-letter');
        }

        /////If the tile is a space, we start the next word on a new row.
      } else {
        words ++;

        //////Set starting point for next word.  This is the remaining spaces between the prior word and the end of the row
        ///// plus the amount of spaces required on the next row to center the next word.
  ...
  aiChooseCorrectLetter(solution) {
    ...
    for (let i = 0; i < solution.length; i++) {
      ...
      && currentPlayer.vowelCheck(solution[i])) {
