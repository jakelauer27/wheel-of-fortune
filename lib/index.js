...
function createPlayers() {
  ...
  players.forEach( (player, i) => {
    var playerInfo = [$(`.input-player${i + 1}-name`).val(), `player1${i + 1}`];
    var playerAiBox = $(`player${i + 1}-ai-checkbox`).attr('checked');
    players[i] = playerAiBox ? new Player(...playerInfo) : new ai-Player(...playerInfo);
  })
}
